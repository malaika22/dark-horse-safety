...
/** Leaf link or nested group (e.g. Accounts / Sales under CRM). */
...
export function isNavGroup(
  child: NavChild,
): child is { id: string; label: string; children: NavLink[] } {
  return Array.isArray(
    (child as { children?: NavLink[] }).children,
  );
}

export function navChildHrefs(child: NavChild): string[] {
  if (isNavGroup(child)) return child.children.map((c) => c.href);
  return [child.href];
}

export function collectNavHrefs(item: NavItem): string[] {
  const hrefs: string[] = [];
  if (item.href) hrefs.push(item.href);
  for (const child of item.children ?? []) {
    hrefs.push(...navChildHrefs(child));
  }
  return hrefs;
}
 ...
 * Accounts, Sales, and Configuration nest under CRM / Customer.
 */
export const APP_NAV: NavItem[] = [
  ...
  },
  {
    ...
    children: [
      ...
      { id: "hr-training", label: "Training", href: "/hr/training" },
      {
        id: "hr-sse",
        label: "SSE Programme",
        href: "/hr/sse-programme",
      },
      {
        id: "hr-on-call",
