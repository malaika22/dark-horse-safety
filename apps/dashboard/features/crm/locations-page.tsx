...
export function LocationsPage() {
  ...
  const [viewMode, setViewMode] = React.useState<"list" | "map" | "split">("split");
  ...
  const [savedViews, setSavedViews] = React.useState<DashboardSavedView[]>(LOCATIONS_SAVED_VIEWS);
  const [activeViewId, setActiveViewId] = React.useState<string | null>("view-1");

  const filteredCards = React.useMemo(() => {
    const q = query.trim().toLowerCase();
    if (!q) return LOCATIONS_CARDS;
    return LOCATIONS_CARDS.filter((card) =>
