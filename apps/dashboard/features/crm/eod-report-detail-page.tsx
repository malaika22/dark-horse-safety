...
function Metric({ label, value }: { label: string; value: string }) {
  return (
    <div className="min-w-0">
      ...
      <p className="mt-1 font-sans text-[13px] font-normal uppercase tracking-[-0.02em] text-[#FDFDFF] md:text-[14px]">
        {value}
      </p>
    </div>
  );
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0 space-y-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <DashboardBadge variant={detail.status.variant} pill>
              {detail.status.label}
            </DashboardBadge>
            <span className="font-sans text-[11px] font-normal uppercase tracking-[-0.02em] text-[#959597] md:text-[12px]">
              {row ? `${row.rep} · ${row.date} · ${row.submittedTime}` : detail.meta}
            </span>
          </div>
        </div>
          ...
          <div className="space-y-3 p-4">
            {detail.pipeline.map((item) => (
              <div key={item.label} className="flex items-center justify-between gap-3">
                ...
                <span className="font-sans text-[12px] font-normal uppercase tracking-[-0.02em] text-[#FDFDFF] md:text-[13px]">
                  {item.value}
                </span>
              </div>
            ))}
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_320px]">
        ...
        <DashboardPanel>
          ...
          <div className="flex flex-col gap-2 p-4">
            <DashboardToolbarButton variant="primary" className="w-full justify-center">
              Approve
            </DashboardToolbarButton>
